...
def plot_weights_given_ax(ax, array,
    ...
    for color in highlight:
        for start_pos, end_pos, similarity, score in highlight[color]:
            ...
            ax.text(start_pos, max_height, 'sim:{:.3f},score:{:.3f}'.format(similarity, score),
...
def plot_weights_given_ax1(ax, array,
                          figsize=(20, 2),
                          height_padding_factor=0.2,
                          length_padding=1.0,
                          subticks_frequency=1.0,
    ...
    # the highlight dict should be the color
    for color in highlight:
        for start_pos, end_pos, similarity, score, tag in highlight[color]:
            assert start_pos >= 0.0 and end_pos <= array.shape[0]
            min_depth = np.min(depths_at_positions[start_pos:end_pos])
            max_height = np.max(heights_at_positions[start_pos:end_pos])
            ax.add_patch(
                matplotlib.patches.Rectangle(xy=[start_pos, min_depth],
                                             ...
                                             edgecolor=color, fill=False))
            #
            ax.text(start_pos, max_height, '{}, sim:{:.3f},score:{:.3f}'.format(tag, similarity, score),
                    color='black', fontsize='xx-small')
...
def plot_weights1(array,
                 figsize=(20, 2),
                 out_f=None,
                 **kwargs):
    fig = plt.figure(figsize=figsize)
    ax = fig.add_subplot(111)
    plot_weights_given_ax1(ax=ax, array=array, **kwargs)
    if out_f is not None:
        plt.savefig(out_f, format='png', bbox_inches='tight', dpi=300)
    else:
        plt.show()